<template>
  <div id="livevideo" :style="{backgroundColor:read ? Colors.textTure : Colors.background,borderRadius:borderRadius}">
    <style>
       :root{
    ...
    <div id="details-cover" v-if="!loading">
      <span class="message">
        <span class="username">{{username}}</span> is shooting a live video now !!!
      </span>
    ...
    <div v-if="!loading">
      <v-menu open-on-hover :close-on-click="false" :close-on-content-click="false" offset-y>
        ...
        <v-card :color="Colors.background" width="100%">
          ...
          <v-list-item link>
            <v-list-item-title :style="{color:Colors.subTextColor}">Do not notify me for live video</v-list-item-title>
          </v-list-item>
          <v-list-item link>
