<template>
  <div id="event" :style="{backgroundColor:read ? Colors.textTure : Colors.background,borderRadius:borderRadius}">
    <style>
       :root{
    ...
    <div id="details-cover" v-if="!loading">
      <span class="message">
        <span class="username">{{username}}</span> Shared an Event holding
        at <b> <nuxt-link to="">{{location}}</nuxt-link>  </b> hosted by
        <b> <nuxt-link to=""> <span class="hosted-by"> {{hostedBy}} </span> </nuxt-link>  </b>
        <br>
       <i> tap to learn more...</i>
      </span>
      ...
      <v-menu open-on-hover :close-on-click="false" :close-on-content-click="false" offset-y>
          ...
          <v-card :color="Colors.background" width="100%">
            <v-list-item link  @click="markRead">
              ...
              <v-list-item link>
              <v-list-item-title :style="{color:Colors.subTextColor}">Remind Me about event</v-list-item-title>
            </v-list-item>
            <v-list-item link>
...
export default {
  ...
  methods: {
    getDetails(id) {
      ...
      this.time = "3 mins ago";
      this.location = "Festust Iyayi Hall"
      this.hostedBy = "Tecno Mobile";
      this.read = false;
      this.loading = false;
    },
    markRead(){
