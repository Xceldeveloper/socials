<template>
  <div class="tag-cover">
    ...
    <span class="tag-header">
      <v-chip color="blue" :text-color="Colors.background">
        <v-avatar left :color="Colors.background">
          <v-icon color="blue" size="20px">mdi-pound</v-icon>
        </v-avatar>
        {{tag}}
      </v-chip>
      {{counts}}
    ...
    >
      <v-slide-item v-for="(tag,index) in exptags" :key="index">
        <nuxt-link :to="tag.link">
          <div class="tag-item-cover">
            <img :src="tag.thumbnail" :alt="tag" />
            <span v-if="tag.multiple" class="tag-item-multiple mdi mdi-view-dashboard"></span>
            <span v-if="tag.type == 'image'" class="tag-item-type mdi mdi-camera-outline"></span>
            <span v-if="tag.type == 'video'" class="tag-item-type mdi mdi-video-outline"></span>
          </div>
        </nuxt-link>
      </v-slide-item>
    </v-slide-group>
    ...
    <div class="special-slide" v-if="isMobile">
       <nuxt-link :to="tag.link" v-for="(tag,index) in exptags" :key="index">
          <div class="tag-item-cover">
            <img :src="tag.thumbnail" :alt="tag" />
            <span v-if="tag.multiple" class="tag-item-multiple mdi mdi-view-dashboard"></span>
            <span v-if="tag.type == 'image'" class="tag-item-type mdi mdi-camera-outline"></span>
