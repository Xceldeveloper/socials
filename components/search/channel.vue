<template>
  <div class="channel-cover" :style="{backgroundColor:Colors.textTure}">
    <style>
       :root{
    ...
    <div id="name-cover" v-if="!loading">
      ...
      <span class="channelCategory">{{channelCategory}}</span>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    getDetails(id) {
      console.log(id);
      (this.channelName = "Drake"),
        (this.channelCategory = "John Doe"),
        (this.avatar = require("~/assets/drake.jpg"));
      this.loading = false;
    }
  }
};
