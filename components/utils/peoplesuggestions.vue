<template>
  <div id="People-suggestion">
    ...
    >
      <v-slide-item v-for="(tag,index) in people" :key="index">
        <div id="person">
          <nuxt-link :to="''">
            <img
              id="avatar"
              img
              :src="tag.avatar"
              :alt="tag.username + 'avatar'"
              oncontextmenu="return false;"
            />
            <span id="username">{{tag.username}}</span>
            <span id="fullname">{{tag.fullname}}</span>
          </nuxt-link>
          <div id="btn-wrapper">
            <v-btn
              v-if="!tag.following"
...
export default {
    ...
    return {
      people: [
        ...
        },
        {
          userId: 2342,
          username: "Birthus",
          fullname: "Uchechkwu Evans fff  ddss s ss s",
          avatar: require("~/static/drake.jpg"),
          following: false
        },
        {
  ...
  methods: {
    follow(index) {
      if (!this.people[index].following) {
        this.$store.commit("user/Follow", {
          username: this.people[index].username,
          fullname: this.people[index].fullname,
          avatar: this.people[index].avatar
        });

        this.people[index].following = true;
      } else {
        ...
        this.people[index].following = false;
      }
    }
  }
};
