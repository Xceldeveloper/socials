...
export default {
  ...
  head() {
    ...
    mounted(){
     var isLoggedIn = this.$store.state.user.isLoggedIn;
        if(!isLoggedIn){
            console.log("fetching from db from callbacks...")
            var iss = false;
            setTimeout(()=>{
              if(iss){
                  this.go('home')
